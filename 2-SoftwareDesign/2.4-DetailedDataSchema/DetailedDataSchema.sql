CREATE TABLE User_n( -- опис користувача full_name VARCHAR(100), -- ПІБ користувача
  date_of_birth DATE, -- Дата народження користувача gender VARCHAR(50), -- Стать користувача
...
CREATE TABLE Request_noise_level( -- опис запиту на вимірювання шуму
  ...
  request_status VARCHAR(50), -- статус запиту user_id INT, -- номер користувача, який надсилає запит
  FOREIGN KEY (user_id) REFERENCES User(user_id) );

CREATE TABLE Answer_on_request( measured_noise_level FLOAT,
  answer_time DATE, answer_status VARCHAR(50),
  requestnoiselevel_id INT, system_id INT,
...
ALTER TABLE User_n ADD ( user_id INT PRIMARY KEY );

ALTER TABLE Request_noise_level ADD ( requestnoiselevel_id INT PRIMARY KEY );

ALTER TABLE Technical_employee ADD CONSTRAINT salary CHECK ( salary > 0 );

-- Додавання нового стовпця до таблиці User_n
ALTER TABLE User_n ADD mobile_phone CHAR(14);
